...
export function loadPage() {
  ...
  taskArray[4] = {
    title: "Quiz",
    description: "Study for your quiz!",
    dueDate: "Saturday",
    priority: 8,
  };
...
export function newProject(e) {
  ...
  projectArray[projectArray.length] = {
    name: this.parentElement.newprojname.value,
  };
  const projcont = document.getElementById("projcont");
...
export function goHome() {
  ...
  for (let i = 0; i < projectArray.length; i++) {
    for (let j = 0; j < projects[i].querySelectorAll("*").length - 2; j++) {
      projects[i].querySelectorAll("*:not(.view):not(.projectname)")[
        j
      ].style.display = "none";
    }
  }
...
function viewProject(e) {
  ...
  this.parentElement.querySelector(".buttonholder").style.justifyContent =
    "space-around";
